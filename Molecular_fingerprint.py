...
filename='E:\Professional\TheProject\Codes\Structures\pdb\\1.pdb'
...
fpnames=pd.DataFrame({'molecular mass':[0],'mlogp':[0],'tpsa':[0],'HAccept':[0],'HDon':[0],'nrotate':[0],'carboxylic acid':[0],'carboxylate':[0],'ester':[0],'alcohol':[0],'primary amine':[0], 'secondary amine':[0],'tertiary amine':[0],'ketone':[0],'aldehyde':[0],'amide':[0],'ether':[0], 'cyanide':[0],'cyanate':[0]})
...
#%% Use SMILES to Calculate MlogP and TPSA (topological polar surface area), , number of hydrogen bond donors, etc for lipophilicity assessment. Look at https://www.rdkit.org/docs/index.html for more parameters
    ...
    if 'O' in pdbdf['atom_name'][i]:
        for j in range(len(pdbdf['atom_name'])):
            if 'C' in pdbdf['atom_name'][j] and dista(i,j)<1.6 and dista(i,j)>1.19 :
                duma=[]
                for k in range(len(pdbdf['atom_name'])):
                    if 'C' in pdbdf['atom_name'][k] and dista(k,i)<1.6 and dista(k,i)>1.19 and k!=j:  
                        for l in range(len(pdbdf['atom_name'])):
                            if (dista(k,l)<1.6 and dista(k,l)>0.5 and l!=i) or (dista(j,l)<1.6 and dista(j,l)>0.5 and l!=i):
                                duma.append(l)
                                duma=list(set(duma))
